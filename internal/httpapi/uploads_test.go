...
func TestCreateUploadDecodesBrowserEscapedLogicalPath(t *testing.T) {
	...
	s, err := repo.UploadSessionByID(context.Background(), out.UploadID)
	if err != nil {
		t.Fatal(err)
	}
	if got, want := s.LogicalPath, "/drop/a+b/%? 空/中文😀.txt"; got != want {
		t.Fatalf("logical path = %q, want %q", got, want)
	}
}
