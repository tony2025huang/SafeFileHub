...
func (s *ObjectStore) Create(logicalPath string) (string, io.WriteCloser, error) {
	...
	objects, shard, err := s.objectDirectory(key, true)
	if err != nil {
		return "", nil, err
	}
...
func (s *ObjectStore) Open(key string) (*os.File, error) {
	...
	objects, shard, err := s.objectDirectory(key, false)
	if err != nil {
		return nil, err
	}
	defer unix.Close(objects)
...
func (s *ObjectStore) objectDirectory(key string, create bool) (objects, shard int, err error) {
	prefix, name, err := parseObjectKey(key)
	if err != nil {
		return -1, -1, err
	}
	objects, err = openDirectoryAt(s.fd, "objects", create)
	if err != nil {
		return -1, -1, err
	}
